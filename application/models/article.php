...
class Article extends CI_Model {
	...
	public function getArticles($limit, $start, $sort) {
        if($sort[5] == "category") {
            $sort[5] = "article_category.name";
        } else {
            $sort[5] = "article.".$sort[5];
        }
    ...
    public function getArticleVisa() {
    ...
    public function getArticleAbout() {
        $this->db->select('*');
        $this->db->where('category_id', 37);
        $query = $this->db->get('article');
        return $query->result();
    }

    public function getArticleOutbound() {
        $this->db->select('*');
        $this->db->where('category_id', 38);
        $query = $this->db->get('article');
        return $query->result();
    }

    public function getArticleVehicleRental() {
        $this->db->select('*');
        $this->db->where('category_id', 39);
        $query = $this->db->get('article');
        return $query->result();
    }
    ...
    public function getArticleBlogs() {
        $this->db->select('*');
        $this->db->where('category_id', 41);
        $query = $this->db->get('article');
        return $query->result();
    }

    public function getArticleGuides() {
        $this->db->select('*');
        $this->db->where('category_id', 35);
        $query = $this->db->get('article');
        return $query->result();
    }
    ...
    public function getArticleCategoryBySlug($cate_slug) {
        $this->db->select('*');
        $this->db->where('url', $cate_slug);
        $query = $this->db->get('article_category');
